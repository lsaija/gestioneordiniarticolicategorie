...
public class MyTestOrdiniArticoliCategorie {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println(
					"In tabella Articolo ci sono " + articoloServiceInstance.listAll().size() + " elementi.");
	...
	private static void testAggiornamentoOrdine(OrdineService ordineServiceInstance) throws Exception {
		...
		if (ordineInstance.getUpdateDateTime().isAfter(updateDateTimeIniziale))
			throw new RuntimeException(
					"testAggiornamentoOrdine fallito: le date di aggiornamento sono disallineate ");
	...
	private static void testAggiornamentoArticolo(ArticoloService articoloServiceInstance,OrdineService ordineServiceInstance) throws Exception {
	...
	private static void testRimozioneArticoliDaOrdine(ArticoloService articoloServiceInstance,OrdineService ordineServiceInstance) throws Exception {
		...
		Ordine ordineReloaded1 = ordineServiceInstance
				.caricaSingoloElementoEagerArticoli(ordineInstance.getId());
		...
		articoloServiceInstance.rimozioneArticoliDaOrdine(ordineInstance.getId());
		
		
		Ordine ordineReloaded2 = ordineServiceInstance
				.caricaSingoloElementoEagerArticoli(ordineInstance.getId());
	...
	private static void testRimuovereCategoriaDaArticolo(ArticoloService articoloServiceInstance,
		...
		articoloServiceInstance.rimuoviCategoria(articoloInstance, categoriaInstance);
		...
		if (!articoloReloaded2.getCategorie().isEmpty())
			throw new RuntimeException("testRimuovereCategoriaDaArticolo fallito: categoria non rimossa ");
	...
	private static void testRimuovereArticoloDaCategoria(ArticoloService articoloServiceInstance,
		...
		categoriaReloaded1 = categoriaServiceInstance
				.caricaSingoloElementoEagerArticoliCategoria(categoriaInstance.getId());
		      if (categoriaReloaded1.getArticoli().isEmpty())
			         throw new RuntimeException("testAggiungereArticoloACategoria fallito: articoli  aggiunti ");
	    articoloReloaded=articoloServiceInstance.caricaSingoloElementoEagerCategorie(articoloInstance.getId());
		
		
		categoriaServiceInstance.rimuoviArticolo(categoriaInstance, articoloInstance);
		...
		System.out.println(categoriaReloaded2.getArticoli().size());
		if (!categoriaReloaded2.getArticoli().isEmpty())
			throw new RuntimeException("testRimuovereArticoloDaCategoria fallito: categoria non rimossa ");
